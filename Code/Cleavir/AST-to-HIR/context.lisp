...
(defun context (results successors invocation)
  (unless (or (and (listp results)
		   (every (lambda (result)
			    (cleavir-ir:lexical-location-p result))
			  results))
	      (cleavir-ir:values-location-p results))
    (error "illegal results: ~s" results))
  (unless (and (listp successors)
	       (<= 1 (length successors) 2)
	       (every (lambda (successor)
			(cleavir-ir:instruction-p successor))
		      successors))
  ...
  (unless (cleavir-ir:enter-instruction-p invocation)
